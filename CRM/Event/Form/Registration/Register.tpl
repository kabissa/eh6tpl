...
{if $priceSet}
    <fieldset id="priceset" class="crm-group priceset-group"><legend>How will you join?</legend>
...
{if $event.event_type_id EQ 7} 
	...
	<fieldset id="priceset" class="crm-group priceset-group"><legend>Who's coming</legend>
		<div class="crm-container">
			<table>
				{foreach from=$eventPartipants.values item=Participant}
					...
					<tr>
						<td colspan='3'>
							{if $Participant.custom_2 ne ''}
								{$Participant.display_name}&nbsp;&#45;&nbsp;{$Participant.custom_2}
								{if $roleLabel ne ''}
									&nbsp;&#40;{$roleLabel}&#41;
								{/if}	
					...
					{if $Participant.custom_1 ne ''}
						<tr>
							<th>Website</th>
							<td>:</td>
							<td><a href="{$Participant.custom_1}">{$Participant.custom_1}</a></td>
						</tr>	
					{/if}
					{* Display blog is not empty *}			
					{if $Participant.custom_4 ne ''}
						<tr>			
							<th>Blog</th>
							<td>:</td>
							<td><a href="{$Participant.custom_4}">{$Participant.custom_4}</a></td>
						</tr>	
					{/if}	
					{* Display comments if not empty *}
					{if $Participant.custom_3 ne ''}
						<tr>			
							<th>Comments</th>
							<td>:</td>
							<td>{$Participant.custom_3}</td>
						</tr>
					{/if}		
				{/foreach}	
			</table>	
...
{* end of Kabissa Redmine issue 442 *}
